Replace the placeholder node renderer with proper shapre rendering and a drag preview

## Implementation

1. Replace the placeholder and node shape rendering. 
    - rectangle, pill, and circle should use CSS styling
    - diamond, hexagon, and cylincer should render with
        SVG shapes
    - keep borders subtle at rest and brighter when selected

2. Add a shape drag preview.
    - when dragging a shape from the shape panel, show a ghost preview of that shapep.
    ...
    - keep this limited to the drag preview behavior only
...
- SVG shapes render and scale correclty for diamond, hexagon, and cylinder.
- Shape dragging shows a ghost preview matching the dragged shape.
- `npm run build` pass type without tupe errors.

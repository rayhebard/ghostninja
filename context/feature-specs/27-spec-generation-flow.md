...
- accept `projectId`, `roomId`, `chatHistory`, `nodes`, and `edges`
- validates input with Zod
- use Gemini through `@ai-sdk/google`
- generates a Markdown technical spec from the canvas and chat context
- updates the run metadata/status for realtime tracking
- return the generated spec content as task output
...
- Check `context/project-overview.md` and `context/architecture-context.md` for system alignment before Implementation
...
- Reuse the existing auth, Prisma, Trigger.dev, and Gemini patterns
...
- `generate-spec` run through Trigger.dev and returns Markdown output

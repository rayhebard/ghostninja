Add shared AI activity indicators so everyone in the room can see when generation is in progress. This unit is only for UI, presence, and realtime status signal. Do not all the actual AI generation flow yet.
...
1. Add AI thinking state to the sidebar.
    ...
    - keep the rest of the sidbar usable

2. Add a shared AI statut feed.
    - check the existing Liveblocks setup and installed agent-related reature first
...
4. Add thinking indicators to live cursor.
    - when a participation has `thinking:true` in presence, show a small spinner in their cursor name badge
    - hide the spinner when `thinking` is false or missing

## Scope Limits

- don't add the actual AI generation logic
- don't trigger background task yet
